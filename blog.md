# Building Ascent on Aurora DSQL + Vercel: a maturity index for AI-native engineering
...
1. **Name:** *Ascent*. You climb a maturity ladder, and levels are rungs.
...
## Entry 4: MVP shape, shipping without a database (on purpose)

The MVP runs on **Vercel** with **zero database**: Next.js 16 route handlers read a repo
over the GitHub REST API (no clone, so it stays serverless-friendly), deterministic analyzers
