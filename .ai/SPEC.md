...
| `agents` | Vendor-neutral registry: `{ id, kind, entrypoint }`. Filled from the guidance formats the repo actually carries. |
| `guidance` | **Optional (0.3.0).** `canonical` (the instruction document that is the AUTHORITY) + `projections[]` of `{ agent, path, generatedFrom, hash }`. See "Guidance" below. |
...
## Guidance: one authority, and projections of it (0.3.0, optional)

A repo that adopted agents from more than one vendor usually carries more than one instruction
document — `CLAUDE.md`, `AGENTS.md`, `.cursorrules`, `.github/copilot-instructions.md`,
`.windsurfrules`. They address the same audience about the same codebase, and nothing in the
ecosystem reads them against each other, so they drift apart quietly. Once they disagree, the answer
an agent gets depends on **which file it happened to open** — which is strictly worse than having one
document, even though it looks like more effort was invested.

The `guidance` block is the repo declaring the answer instead of every reader guessing it:

```yaml
guidance:
  canonical: AGENTS.md
  projections:
    - { agent: cursor, path: .cursorrules, generatedFrom: AGENTS.md, hash: "0a2e6982ba22" }
    - { agent: copilot, path: .github/copilot-instructions.md, generatedFrom: AGENTS.md, hash: "0a2e6982ba22" }
```

- **`canonical`** is the one document to believe when two of them disagree.
- **`projections[]`** are files GENERATED from it. `node .ai/maintain.mjs project` renders each one —
  the canonical body verbatim, under a provenance header, with vendor front matter first where the
...
```
<!-- generated-from: AGENTS.md sha256:0a2e6982ba22 · body: sha256:0a2e6982ba22 · do not edit; run: node .ai/maintain.mjs project -->
```

| Comparison | Meaning | Doctor |
| --- | --- | --- |
| source hash ≠ the canonical's current hash | the canonical moved on; this file is BEHIND | `warn` — nothing is wrong, it is out of date |
| body hash ≠ this file's own body hash | somebody edited the PROJECTION instead of the source | `fail` — the repo now has two sources of truth |
| declared projection missing | it was never generated, or was deleted | `fail` |
| a guidance file that is neither canonical nor declared | an ungoverned second opinion nothing keeps in step | `warn` |

**Absent is not failing.** A repo with no `guidance` block reports `unchecked` for this whole check —
it has not adopted the block, which is a different statement from having adopted it and drifted.
...
6. **Guidance projections**: every file declared under `guidance.projections` still matches the
   canonical document it was generated from. A missing declared projection is a **fail**; a projection
   generated from an older canonical is a **warn** (it is behind, nothing is wrong); a projection whose
   body no longer matches its own recorded hash is a **fail** (it was hand-edited, so the repo now has
   two sources of truth); a guidance file that is neither the canonical nor a declared projection is a
   **warn**. No `guidance` block at all is **unchecked** — a repo that has not adopted it is not
   failing it.
7. **Guardrails**: the invariants in `guardrails.yaml` that a machine CAN check are checked: no file
   matching `secrets.neverCommit` may be tracked by git. A violation is a hard failure.
8. **Score**: prints a conformance percentage and the projected maturity delta, so the agent gets a
...
`unchecked` — the clause could not be judged in this environment (checks 5 and 7 both need git
...
A reimplementation in another language is conformant if it performs checks 1–7 against this spec
...
| `guidance.canonical` | `guidance.canonical` names a file that resolves |
| `guidance.<path>` | that declared projection is present, header-stamped, unedited and in sync |
| `guidance.unchecked` | the manifest declares no `guidance` block, so drift was not judged |
...
- Adding an optional field, capability name, or finding level → **patch/minor**, no reader changes.
  ...
  `findings[]` and reads the same summary numbers. v0.3.0 also adds the OPTIONAL `guidance` block and
  its projection-drift check; a v0.2.0 manifest simply does not carry it, the check reports
  `unchecked`, and a v0.2.0 reader ignores the field — which is exactly what "additive within a
  major" is supposed to feel like.)
