...
- [ ] **Check `LLM_TOTAL_BUDGET_MS` / `LLM_TIMEOUT_MS`** for the chosen provider. The total-budget default is now **provider-aware**: **90s** for hosted models (fine for Flash) and **15 min** for `claude-cli` (which also self-defaults `CLAUDE_CLI_TIMEOUT_MS` to 10 min) so a self-hosted claude-cli deploy stays live instead of silently degrading to the mock floor. An explicit `LLM_TOTAL_BUDGET_MS` still overrides. _(Note: `SCAN_CLIENT_TIMEOUT_MS=720_000` in `src/components/report/scanEstimate.ts` is a generous client backstop, calibrated for slow scans — lower it if Flash is consistently fast.)_
