...
references:
  ...
  - https://zylo.com/blog/finops-cost-optimization — ~50% of bought SaaS licenses/seats go unused; the discipline is matching committed spend to observed utilization. Sets the bar: "150/mo included" only earns its keep if the tool shows me I'm using a defensible fraction of it — under 20% means downgrade, over 100% means I'm being shoved to the next tier.
---

> **Overlay correction 2026-08-31 (`/uat drain 2026-08-30-moonshot-cert`).** This sheet said
> **Team = 500 credits/mo** throughout; the plan was repriced to **150** on 2026-08-19
> (`src/lib/plans.ts:234`, confirmed live on `/usage`). Every occurrence is now 150. The
> 2026-08-30 walker correctly took the denominator from the code and left this file alone
> (`VICTOR-L1-01` scope_note); the drain is where it lands. His utilization arithmetic — the one
> number that decides the tier — was otherwise wrong by 3.3x in every future run.

## Who they are
Victor is an Engineering Director at a ~300-engineer, mixed-stack company (TS/Go/Python/some JVM) who also carries the engineering-SaaS budget line — he is the person whose name is on the Ascent renewal. He's on **Team (150 credits/mo)** and runs a **weekly fleet scan** across the private repos that matter. He thinks in unit economics: every recurring tool gets a row in his spreadsheet — cost, what it replaced, and a defensible "kept it / cut it" each quarter. He is not evaluating whether Ascent is real (he's past that); he's deciding whether the *repetition* is right-sized and whether each cycle's insight justifies the credits it burns.
...
## Voice
Unit-economics first, dry, numerate. "What's my burn against the 150?" "Show me utilization, not a balance." He says "right-size," "headroom," "idle spend," "$/scan," "is this rolling over or am I lighting credits on fire monthly." When a dashboard shows a balance but not "X of 150 used," he says "that's a fuel gauge with no tank size — useless for budgeting." His highest praise is "okay, that's a row I can defend to finance." His killshot is "I can't compute cost-per-value from this, so I'll model it conservatively and probably downgrade." He won't say "great insights"; he'll say "is *this* cycle worth one credit more than last cycle."

## Jobs to be done
- Each cycle, see **credits burned this period vs. the 150 included** — am I right-sized, leaving credits idle (overpaying for headroom), or blowing past into a forced upgrade?
...
- `/usage` shows burn **against the allotment** — "X of 150 used this period (Y%)" — not just a raw billable count and a balance. Per the FinOps right-sizing bar, utilization-vs-commitment is the single number that decides the tier.
...
- A **balance without a tank size** — credits remaining but no "of 150," so I can't see utilization. Instant "useless for budgeting."
- "**150 / month**" copy when the underlying field is a **persisted pool** — if it actually rolls over, calling it "per month" undersells it; if it resets, the unused headroom is silent waste. Either way the ambiguity is a finding.
