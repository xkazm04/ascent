...
grounding: 4/5 in-direction (P0-2 stack-fit + P0-3 priority-ranked files FIXED 2026-06-20 — up from 3.5; live-confirm pending); OUT-direction CLOSED (P1-5 2026-06-20 — the audit CSV always HAD an engine-provider column; added an engineModel column + a keyless-default floor caveat). No cross-scan memory (still open). Session [[2026-06-20-tiger-l1]]
...
- **Grounding on the way OUT — mostly a false alarm, now fully closed (P1-5).** The L1 claim "the signed CSV has no engine column" was **refuted** (the `engineProvider` column predates this run — commit `a1025c2`), so a mock-degraded quarter was already distinguishable. The genuine gaps — no *model*-level column and a keyless-default scan that disclosed the floor only via a quiet chip — were fixed (added an `engineModel` CSV column + a loud keyless caveat in `scan.ts`). Still, the principle holds: grounding must be audited in **both** directions (into the prompt AND out to the durable artifact) — and an L1 "X is missing" must be code-verified before action.
