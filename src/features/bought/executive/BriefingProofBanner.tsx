...
export function BriefingProofBanner({ proof, loopProof, className = "" }: {
  ...
  return (
    <div className={`rounded-xl border border-emerald-400/30 bg-emerald-400/[0.06] px-4 py-3 ${className}`}>
      {line && (
        <p>
          <span className="type-mono-sm uppercase tracking-widest text-emerald-300">Proof: improvement shipped</span>{" "}
          <span className="type-body text-slate-200">{line}</span>{" "}
          <span className="type-caption text-slate-500">fleet-wide</span>
        </p>
      )}
      {loopLine && (
        <p className={line ? "mt-2" : undefined}>
          <span className="type-mono-sm uppercase tracking-widest text-emerald-300">Local loop</span>{" "}
          <span className="type-body text-slate-200">{loopLine}</span>
        </p>
      )}
    </div>
  );
