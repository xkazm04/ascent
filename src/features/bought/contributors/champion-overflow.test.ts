...
describe("contributors champion card truncation (G6-23)", () => {
  it("wraps the login in a min-w-0 flex-1 truncate cell", () => {
    expect(SOURCE).toMatch(/className="min-w-0 flex-1 truncate font-mono type-body text-white"/);
  });

  it("keeps the rank badge from shrinking so it never gets squeezed off", () => {
    expect(SOURCE).toMatch(/className="shrink-0 type-mono-sm uppercase tracking-widest text-accent"/);
  });
});
