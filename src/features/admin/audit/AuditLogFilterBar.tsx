  ...
  return (
    <div className="mb-3 flex flex-wrap items-end justify-between gap-2">
      ...
      <form onSubmit={onSubmit} className="flex flex-wrap items-end gap-2">
        <label className="flex items-center gap-2 type-body text-slate-400">
          <span className="type-mono-sm uppercase tracking-widest text-slate-500">Action</span>
          <select
            value={action}
        ...
        <label className="flex items-center gap-1.5 type-mono-sm text-slate-500">
          ...
          <input type="date" value={since} onChange={(e) => onChangeSince(e.target.value)} aria-label="From date"
            className="rounded-md border border-slate-700 bg-slate-950 px-2 py-1 type-body-sm text-slate-200 outline-none focus:border-accent" />
        </label>
        <label className="flex items-center gap-1.5 type-mono-sm text-slate-500">
          until
        ...
        <input type="text" value={actor} onChange={(e) => onChangeActor(e.target.value)} placeholder="actor (login)" aria-label="Filter by actor"
          className="w-32 rounded-md border border-slate-700 bg-slate-950 px-2 py-1 type-mono-sm text-slate-200 outline-none focus:border-accent" />
        <button type="submit" disabled={loading}
          className="rounded-md border border-slate-700 px-2.5 py-1 type-mono-sm text-slate-300 transition hover:border-accent hover:text-white disabled:opacity-50">
          Apply
        </button>
      </form>
