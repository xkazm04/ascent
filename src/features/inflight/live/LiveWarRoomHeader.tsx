  ...
  return (
    <>
      ...
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 type-mono-sm uppercase tracking-[0.3em] text-accent">
        ...
        <div className="flex flex-col items-end gap-2">
          <div className="flex items-center gap-2">
            {!readOnly && running && (
              <button
                type="button"
                onClick={onStop}
                className="focus-ring rounded-lg border border-slate-700 px-3 py-2 type-body text-slate-300 transition hover:border-slate-500 hover:text-white"
              >
                Stop
              </button>
            )}
            {!readOnly && onLaunch && (
              <button
                ...
                className="focus-ring rounded-lg bg-accent px-4 py-2 type-body font-semibold text-on-accent transition hover:bg-accent-soft disabled:cursor-not-allowed disabled:opacity-50"
              >
                {launchLabel}
              </button>
            )}
            <button
              ...
              className="focus-ring rounded-lg border border-slate-700 px-3 py-2 type-body text-slate-300 transition hover:border-accent hover:text-white"
            >
              ⛶ TV mode
            </button>
            {canShare && !readOnly && (
              <button
                ...
                className="focus-ring rounded-lg border border-slate-700 px-3 py-2 type-body text-slate-300 transition hover:border-accent hover:text-white disabled:opacity-50"
              >
          ...
          {!readOnly &&
            (watchedCount === 0 ? (
              <p className="type-mono-sm text-slate-500">Watch some repos in Repositories to scan.</p>
            ) : (
              ...
              <p className="type-mono-sm text-slate-500" aria-live="polite" aria-atomic="true" suppressHydrationWarning>
                {running
          ...
          {readOnly && fleetScannedAt && (
            <p className="type-mono-sm text-slate-500" suppressHydrationWarning>
          ...
          {!readOnly && onToggleSound && (
            <label className="flex items-center gap-1.5 type-mono-sm text-slate-500" title="Play a short chime when a repo crosses into AI-Native">
