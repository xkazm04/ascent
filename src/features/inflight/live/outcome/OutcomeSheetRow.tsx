      ...
      {columns.map((col) => {
        ...
        return (
          <td key={col.id} className="border-b border-l border-divider bg-surface/60 px-2 py-2 align-top">
            ...
            {/* NO BASELINE, as a WORD beside the verdict and never a panel — the lane rail's own
                ...
                therefore unverified". It says `no baseline` and not `baseline red`, which was read
                as a claim that the repository's checks fail — they may well pass; the worktree the
                guard measured in carries none of the local state a check may need. The full guard
                note is on hover, where "why" survives the throwaway worktree it happened in. */}
            {cell?.redBaseline && (
              <p
                data-testid="cell-baseline-unavailable"
