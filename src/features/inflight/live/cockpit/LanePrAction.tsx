...
export function LanePrAction({ slug, lane, canOpen }: LanePrActionProps) {
  ...
  if (prUrl) {
    return (
      <a href={prUrl} className="focus-ring rounded type-caption text-accent hover:text-accent-soft">
        PR #{lane.prNumber ?? ""} →
      </a>
    );
  }
  ...
  if (!open) {
    return (
      <button
        ...
        className="focus-ring rounded type-caption text-accent hover:text-accent-soft"
      >
        Open PR from lane
      </button>
    );
  ...
  return (
    <div className="mt-2 rounded border border-divider bg-surface-strong/60 p-2">
      <p className="type-caption leading-relaxed text-slate-400">
      ...
      <div className="mt-2 flex flex-wrap items-center gap-2">
        <input
          ...
          className="focus-ring min-w-0 flex-1 rounded border border-divider bg-ink px-2 py-1 type-caption text-slate-200"
        />
        <button
          type="button"
          disabled={busy || confirm.trim() !== lane.repoFullName}
          onClick={() => void submit()}
          className="focus-ring rounded border border-accent/60 px-2 py-1 type-caption text-accent hover:bg-accent/10 disabled:cursor-not-allowed disabled:opacity-40"
        >
          Push and open
        </button>
        <button
