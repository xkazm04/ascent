...
export function TimeRangeSelector({ range, from, to }: { range: RangeKey; from?: string; to?: string }) {
  ...
  const inputCls =
    "rounded-md border border-slate-800 bg-slate-950/60 px-2 py-1 type-mono-sm text-slate-200 [color-scheme:dark] focus:border-accent focus:outline-none";
      ...
      {(customOpen || range === "custom") && (
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            onClick={applyCustom}
            disabled={!fromVal}
            className="rounded-md border border-slate-700 px-2.5 py-1 type-mono-sm text-slate-200 transition hover:border-accent hover:text-white disabled:cursor-not-allowed disabled:opacity-40"
          >
            Apply
          </button>
        </div>
