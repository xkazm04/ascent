...
export function GatePolicyEditor({ org, initial }: { org: string; initial: GatePolicy | null }) {
  ...
  return (
    <div className="mt-4 border-t border-slate-800 pt-4">
      <div className="type-mono-sm uppercase tracking-widest text-accent">Edit policy</div>
      <div className="mt-2 grid gap-3 sm:grid-cols-2">
        <label className="flex items-center justify-between gap-2 type-body-sm text-slate-400">
          Minimum level
          <select
            value={f.minLevel}
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2" aria-busy={f.busy !== null}>
        ...
        <button
          onClick={f.reset}
          disabled={f.busy !== null}
          className="rounded-md border border-slate-700 px-3 py-1.5 type-body-sm text-slate-400 transition hover:border-orange-400 hover:text-orange-300 disabled:opacity-50"
        >
          Reset to default
        </button>
      </div>
      ...
      <div role="status" aria-live="polite" className="mt-2">
        ...
        <p className="mt-1 type-body-sm text-slate-500">{f.applies ?? ""}</p>
      </div>
    </div>
  );
}
