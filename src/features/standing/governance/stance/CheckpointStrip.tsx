      ...
      {cols.map((col) => (
        <div key={col.key} className="bg-ink p-4">
          <div className="flex items-baseline justify-between gap-2">
            <span className="font-mono type-micro uppercase tracking-[0.18em]" style={{ color: col.hex }}>
              {col.title}
            </span>
            <span className="font-mono type-lede tabular-nums" style={{ color: col.hex }}>
              {col.items.length}
            </span>
          </div>
          <p className="mt-1 type-body-sm text-slate-500">{col.copy}</p>
          <ul className="mt-3 space-y-1.5">
            {col.items.map((it) => (
              <li key={it.label} className="flex items-baseline justify-between gap-2">
                <span className="type-mono-sm text-slate-100">{it.label}</span>
                {it.note && <span className="font-mono type-micro tabular-nums text-slate-600">{it.note}</span>}
              </li>
            ))}
            {col.items.length === 0 && (
              <li className="type-body-sm text-slate-600">{col.key === "undeclared" ? "Nothing undeclared observed." : "None declared."}</li>
            )}
          </ul>
        </div>
      ))}
