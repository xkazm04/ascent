...
const FIELD =
  "w-full rounded-md border border-slate-700 bg-slate-950 px-2 py-1 type-mono-sm text-slate-200 outline-none focus:border-accent";
...
export function StanceEditor({
  ...
  return (
    <div className="mt-4 border-t border-slate-800 pt-4">
      <div className="type-mono-sm uppercase tracking-widest text-accent">Edit stance</div>
      <div className="mt-2 grid gap-3 sm:grid-cols-2">
        <label className="block type-body-sm text-slate-400">
      ...
      <div className="mt-4">
        <div className="flex items-center justify-between">
          <span className="type-body-sm text-slate-400">No-AI zones</span>
          <button
            onClick={f.addZone}
            className="focus-ring rounded-md border border-slate-700 px-2 py-1 font-mono type-micro uppercase tracking-[0.14em] text-slate-400 transition hover:border-accent hover:text-white"
          >
            + zone
          </button>
        </div>
        {f.zones.length === 0 && <p className="mt-1 type-body-sm text-slate-600">No zones declared.</p>}
        <div className="mt-2 space-y-2">
          {f.zones.map((z, i) => (
            <div key={i} className="grid gap-2 rounded-lg border border-divider bg-ink/60 p-3 sm:grid-cols-[1fr_1fr_1fr_auto]">
              <label className="block type-micro text-slate-500">
                Repo globs
              ...
              <button
                ...
                className="focus-ring self-end rounded-md border border-slate-700 px-2 py-1 font-mono type-micro text-slate-500 transition hover:border-orange-400 hover:text-orange-300"
              >
                ✕
              </button>
            </div>
      ...
      <div className="mt-4">
        <span className="type-body-sm text-slate-400">Review requirements by autonomy tier</span>
        <div className="mt-2 grid gap-2 sm:grid-cols-2">
          {TIER_ORDER.map((tier) => (
            <label key={tier} className="block type-micro text-slate-500">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2" aria-busy={f.busy !== null}>
        <button
          ...
          className="rounded-md border border-slate-700 px-3 py-1.5 type-body-sm text-slate-300 transition hover:border-accent hover:text-white disabled:opacity-50"
        >
      ...
      <div role="status" aria-live="polite" className="mt-2">
        <span className={`type-mono-sm ${f.msg?.kind === "error" ? "text-orange-300" : "text-emerald-300"}`}>
