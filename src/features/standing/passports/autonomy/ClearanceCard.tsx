    ...
    <Surface className="flex flex-col gap-4 p-5">
      ...
      <div className="flex items-start justify-between gap-3 border-b border-divider pb-3">
        <div className="min-w-0">
          <Link href={reportPermalink(repo.fullName)} className="font-mono type-body text-white hover:text-accent">
            {repo.name}
          </Link>
          <p className="mt-1 line-clamp-2 type-body-sm text-slate-400">{repo.purpose || "No stated purpose."}</p>
        </div>
        ...
        >
          <div className="type-figure font-medium" style={{ color: hex }}>
            {meta.code}
          </div>
      ...
      <div>
        ...
        <p className="mt-1.5 type-body text-slate-100">{meta.grant}</p>
        <p className="mt-1 type-body-sm text-slate-400">{meta.blurb}</p>
      </div>
      ...
      <div>
        ...
        <ul className="mt-2 space-y-1.5">
          {repo.gates.map((g) => (
            <li key={g.id} className="flex items-start gap-2 type-body-sm">
        ...
        {next ? (
          <>
            <div className="flex items-baseline justify-between gap-2">
              ...
              <span className="type-caption tabular-nums text-slate-400">{repo.nextProgress}%</span>
            </div>
            <Meter
          ...
          <p className="type-body-sm" style={{ color: hex }}>
            Top clearance held. This repo can run agents unattended.
          </p>
        )}
        <p className="mt-3 type-label tracking-[0.18em] text-slate-600">
