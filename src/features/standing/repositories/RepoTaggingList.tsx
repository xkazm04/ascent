  ...
  return (
    <div className="mt-6 border-t border-slate-800 pt-4">
      <div className="flex items-center justify-between gap-3">
        <h3 className="type-mono-sm uppercase tracking-widest text-slate-400">Tag repositories</h3>
        <input
          value={filter}
      ...
      <div className="mt-3 max-h-96 space-y-1.5 overflow-y-auto pr-1">
        {visibleRepos.map((r) => {
          ...
          return (
            <div key={r.fullName} className="flex flex-wrap items-center gap-2 rounded-lg border border-slate-800 bg-slate-950/40 px-3 py-2">
              <span className="min-w-0 flex-1 truncate type-mono-sm text-slate-300" title={r.fullName}>
                {r.fullName}
              </span>
                ...
                {segments.map((s) => {
                  ...
                  return (
                    <button
                      ...
                      aria-pressed={on}
                      className="inline-flex items-center gap-1 rounded-full border px-2 py-0.5 type-mono-sm transition"
                      style={
                        on
        ...
        {visibleRepos.length === 0 && <p className="type-body-sm text-slate-500">No repos match “{filter}”.</p>}
      </div>
      <p className="mt-2 type-mono-sm text-slate-600">{segments.length} segment{segments.length === 1 ? "" : "s"} · {repos.length} repos</p>
    </div>
  );
}
