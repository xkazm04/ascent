...
// 200-LOC cap (AGENTS.md). Owns no JSX. The tagging toggle lives in RepoSegmentsPanel.tagging.ts for
// the same cap; it is handed this hook's state + setters and behaves exactly as it did inline.

import { useMemo, useState } from "react";
import { useAutoAdd } from "./RepoSegmentsPanel.autoAdd";
import { createToggleTag } from "./RepoSegmentsPanel.tagging";
...
export function useRepoSegmentsPanel({
  ...
  const toggle = createToggleTag({ slug, membership, setMembership, setSegments, setError });
