      ...
      <ol className="divide-y divide-divider border-y border-divider">
        {shown.map((s) => (
          <Entry key={s.id} step={s}>
            ...
            {s.id === "permissions" && s.state === "blocked" && view.capabilities.installUrl ? (
              <RegistryButton href={view.capabilities.installUrl}>Grant contents:write ↗</RegistryButton>
            ) : null}
            ...
            {s.id === "point" && s.state === "active" ? (
              <p className="max-w-2xl text-sm text-slate-400">
                ...
                <code className="font-mono text-slate-300">{view.howTo.syncCmd}</code> — that is what makes a repo count as
                pointing.
              </p>
            ) : null}
          </Entry>
