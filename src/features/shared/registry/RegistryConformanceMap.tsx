  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        ...
        <span className="font-mono text-xs text-slate-500">{reading.headline}</span>
      </div>
      ...
      {matrix.empty ? (
        ...
        <Surface radius="xl" className="overflow-x-auto p-0">
          <table className="w-full border-collapse text-sm" style={{ minWidth: `${240 + matrix.repos.length * 92}px` }}>
            <caption className="sr-only">Conformance of each repository against each governing subject</caption>
            <thead>
              <tr className="font-mono text-xs uppercase tracking-[0.16em] text-slate-500">
              ...
              {rows.map((s) => (
                <tr key={s.slug}>
                  <th scope="row" className="bg-ink px-4 py-1.5 text-left font-normal text-slate-300">
                    ...
                    <span className="ml-2 font-mono text-xs text-slate-600">{s.deviations || ""}</span>
                  </th>
                  {matrix.repos.map((repo) => {
                    ...
                    return (
                      <td key={repo} className={`px-2 py-1.5 text-center font-mono text-sm ${CELL_CLASS[cell.state]}`} title={title}>
