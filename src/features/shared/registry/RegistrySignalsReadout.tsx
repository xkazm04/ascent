      ...
      >
        {rows.slice(0, MAX_ROWS).map((s) => (
          <tr key={`${s.bundle}/${s.subjectSlug}`}>
            <td className="px-4 py-2 font-mono text-sm text-slate-300" title={s.bundle}>
              {s.subjectSlug}
            </td>
            <td className="px-4 py-2 text-right font-mono text-sm tabular-nums text-slate-200">{readCount(s.consults)}</td>
            <td className="px-4 py-2 text-right font-mono text-sm tabular-nums text-slate-200">{readCount(s.deviations)}</td>
            ...
            <td className="px-4 py-2 text-right font-mono text-sm tabular-nums text-slate-500">{s.contributors}</td>
          </tr>
        ))}
      </OrgTable>
