// Shared markup for the fidelity scene's regions. No hooks, no "use client": a Region is the
...
/** The tier as a chip: the ordinal's colour is the ramp's meaning (rich → lean), nothing else. */
export function TierChip({ tier }: { tier: Tier }) {
  const tone = tier === "full" ? "border-accent text-accent-soft" : tier === "reduced" ? "border-slate-500 text-slate-200" : "border-slate-700 text-slate-400";
  return (
    <span data-tier={tier} className={`inline-block rounded-md border px-2 py-0.5 type-caption uppercase tracking-[0.15em] ${tone}`}>
      {tier}
    </span>
  );
}
