// Shared markup for the design-tokens scene's regions. No hooks, no "use client": a Region is the
// `data-technique` boundary the frame spotlights, plus the labelled readouts and buttons the panels share.
...
/** A labelled chip group: one pressed choice among a closed set. */
export function Choice<T extends string>({ label, value, options, onChange }: { label: string; value: T; options: readonly T[]; onChange: (v: T) => void }) {
  return (
    <div className="flex flex-wrap items-center gap-2" role="group" aria-label={label}>
      ...
      {options.map((o) => (
        <button key={o} type="button" className={o === value ? BTN_ON : BTN} aria-pressed={o === value} onClick={() => onChange(o)}>
          {o}
        </button>
      ))}
    </div>
...
export const TH = "font-normal text-left text-slate-500";
