...
import { MemoryCard } from "@/features/shared/memory/MemoryCard";
          ...
          <Fragment key={m.id}>
            <tr onClick={() => setExpanded(open ? null : m.id)} className="cursor-pointer">
              ...
              <td className="max-w-0 px-3 py-2">
                ...
                {/* Only once a registry is mapped: before that every note is hosted and the tag is noise. */}
                {registryBase && (
                  <span className="mt-1 inline-block">
                    <OriginTag origin={m.origin} path={m.registryPath} />
                  </span>
                )}
              </td>
