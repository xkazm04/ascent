...
import { PracticeApply } from "@/features/shared/practices/PracticeApply";
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="space-y-3 type-body">
          <div>
            <span className="type-mono-sm uppercase tracking-widest text-slate-500">Learn from</span>
            {p.exemplar ? (
              <div className="mt-1">
                <Link
                  href={`/report?repo=${encodeURIComponent(p.exemplar.fullName)}`}
                  className="font-mono type-body text-white hover:text-accent"
                >
                  {p.exemplar.name}
                </Link>
                <span className="ml-2 type-mono-sm" style={{ color: scoreHex(p.exemplar.score) }}>
                  {p.exemplar.score}/100
                </span>
              </div>
            ) : (
              <div className="mt-1 type-body-sm text-slate-500">No strong exemplar yet. This is greenfield for the org.</div>
            )}
          </div>
          <div>
            <span className="type-mono-sm uppercase tracking-widest text-slate-500">
            ...
            {p.gapRepos.length > 0 ? (
              <div className="mt-1 flex flex-wrap gap-1.5">
                {p.gapRepos.slice(0, 12).map((r) => (
                  <span key={r} className="rounded border border-orange-500/30 bg-orange-500/5 px-1.5 py-0.5 type-mono-sm text-orange-200">
                    {r}
                  </span>
                ))}
                {p.gapRepos.length > 12 && <span className="type-mono-sm text-slate-600">+{p.gapRepos.length - 12}</span>}
              </div>
            ) : (
              <div className="mt-1 type-body-sm text-slate-500">No clear gaps: well adopted across the fleet.</div>
            )}
          </div>
        </div>
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-4">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="type-mono-sm uppercase tracking-widest text-accent">Reusable shape</div>
            ...
            {onPromote && (
              <button
                ...
                className="focus-ring rounded-lg border border-accent/50 bg-accent/10 px-2.5 py-1 type-body-sm font-medium text-white transition hover:bg-accent/20"
              >
                Save as playbook →
              </button>
            )}
          </div>
          <ul className="mt-2 space-y-1 type-body text-slate-300">
            {p.starter.map((s, i) => (
