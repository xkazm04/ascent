    ...
    <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-4">
      <div className="flex flex-wrap items-start justify-between gap-2">
        ...
        <div className="min-w-0 break-words">
          ...
          {p.version > 1 && (
            <span className="ml-2 type-mono-sm text-slate-500" title={`Last edited ${p.updatedAt.slice(0, 10)}`}>
              v{p.version}
            </span>
          )}
        </div>
        <div className="flex shrink-0 items-center gap-2">
          ...
          <button onClick={onRemove} className="focus-ring rounded type-mono-sm text-slate-600 hover:text-orange-300">remove</button>
        </div>
      </div>
      {p.summary && <p className="mt-1 type-body text-slate-400">{p.summary}</p>}
      {p.steps.length > 0 && (
        <ul className="mt-2 space-y-1 type-body-sm text-slate-300">
          {p.steps.map((s, i) => (
      ...
      <details className="group mt-2">
        ...
        <pre className="mt-2 max-h-60 overflow-auto rounded-lg border border-slate-800 bg-slate-950/60 p-3 type-caption whitespace-pre-wrap text-slate-300">
      ...
      {c.markError && <p role="alert" className="mt-2 type-body-sm text-orange-300">{c.markError}</p>}
      {c.prError && <p role="alert" className="mt-2 type-body-sm text-orange-300">{c.prError}</p>}
      {c.prResult && (
