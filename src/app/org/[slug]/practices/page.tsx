          ...
          <Card key={p.id}>
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <div className="shrink-0 text-right">
                ...
                <div className="font-mono text-[10px] uppercase tracking-widest text-slate-500">repos strong</div>
              </div>
            </div>
            ...
            <div className="mt-4 grid gap-4 lg:grid-cols-2">
              ...
              <div className="space-y-3 text-sm">
                <div>
                  <span className="font-mono text-[10px] uppercase tracking-widest text-slate-500">Learn from</span>
                  {p.exemplar ? (
                    <div className="mt-1">
                      <Link href={`/report?repo=${encodeURIComponent(p.exemplar.fullName)}`} className="font-mono text-sm text-white hover:text-accent">
                        {p.exemplar.name}
                      </Link>
                      <span className="ml-2 font-mono text-xs" style={{ color: scoreHex(p.exemplar.score) }}>
                        {p.exemplar.score}/100
                      </span>
                    </div>
                  ) : (
                    <div className="mt-1 text-xs text-slate-500">No strong exemplar yet — this is greenfield for the org.</div>
                  )}
                </div>
                <div>
                  ...
                  {p.gapRepos.length > 0 ? (
                    <div className="mt-1 flex flex-wrap gap-1.5">
                      {p.gapRepos.slice(0, 10).map((r) => (
                        <span key={r} className="rounded border border-orange-500/30 bg-orange-500/5 px-1.5 py-0.5 font-mono text-[11px] text-orange-200">
                          {r}
                        </span>
                      ))}
                      {p.gapRepos.length > 10 && <span className="font-mono text-[11px] text-slate-600">+{p.gapRepos.length - 10}</span>}
                    </div>
                  ) : (
                    <div className="mt-1 text-xs text-slate-500">No clear gaps — well adopted across the fleet.</div>
                  )}
                </div>
              </div>
              ...
              <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-4">
                <div className="font-mono text-[10px] uppercase tracking-widest text-accent">Reusable shape</div>
