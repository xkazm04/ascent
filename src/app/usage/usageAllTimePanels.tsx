  ...
  return (
    <Surface className="mt-6 p-6">
      ...
      <div className="mt-3 grid gap-4 lg:grid-cols-2">
        <div>
          <div className="type-mono-sm uppercase tracking-widest text-slate-500">Monthly-quota denials</div>
          <div className="mt-2 space-y-1.5 type-body">
            {quotaEvents.quotaDenies.length === 0 ? (
              ...
              quotaEvents.quotaDenies.map((d) => (
                <div key={d.scope} className="flex items-center justify-between gap-3">
                  <span className="type-mono-sm text-slate-300">{d.scope === "user" ? "signed-in" : "anonymous"}</span>
        ...
        <div>
          <div className="type-mono-sm uppercase tracking-widest text-slate-500">Rate-limit trips</div>
          <div className="mt-2 space-y-1.5 type-body">
            {quotaEvents.rateLimitTrips.length === 0 ? (
              ...
              quotaEvents.rateLimitTrips.map((t) => (
                <div key={t.scope} className="flex items-center justify-between gap-3">
                  <span className="type-mono-sm text-slate-300">{t.scope}</span>
