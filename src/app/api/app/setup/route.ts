...
// store the installation, then bounce the user to /onboarding for that org.
...
export async function GET(request: Request) {
  ...
  if (authOn && !actorLogin) {
    return NextResponse.redirect(new URL("/onboarding?error=auth_required", request.url));
  }
  ...
  try {
    ...
    if (actorLogin) {
      ...
      if (!authorized) {
        ...
        return NextResponse.redirect(new URL("/onboarding?error=forbidden", request.url));
      }
    }
    ...
    return NextResponse.redirect(new URL("/onboarding?error=setup_failed", request.url));
  }
}
