...
export async function POST(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const stepsChecklist = playbook.steps.length
    ? playbook.steps.map((s) => `- [ ] ${s}`).join("\n")
    : "- [ ] <!-- TODO: break this playbook into concrete steps for this repo -->";

  const fileBody = `# Playbook: ${playbook.title}

> Org standard for **${playbook.dimId} · ${dimLabel}**, rolled out from Ascent. Track adoption by
