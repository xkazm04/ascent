...
export async function POST(request: Request) {
  ...
  const body = (await request.json().catch(() => ({}))) as { org?: string; range?: string; from?: string; to?: string };
  if (!body.org) return NextResponse.json({ error: "Provide { org }." }, { status: 400 });
  const denied = await requireOrgRole(body.org, "owner");
  if (denied) return denied;
  const minted = signBriefingShareToken({ org: body.org, range: body.range, from: body.from, to: body.to });
