...
describe("GET /api/report/skill — authorized happy path + filename sanitization", () => {
  ...
  it("selects no tracks and no cap when neither ?tracks/?dims nor ?max is supplied (default track selection)", async () => {
    await get("acme/api");
    // An EMPTY selection, not a populated one: selectTracks defaults `opts` to {}, so passing the
    // empty object and passing nothing are the same call. What must not happen is a stray include
    // or max sneaking in and silently narrowing the default weak-dimension set.
