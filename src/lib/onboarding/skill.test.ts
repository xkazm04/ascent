...
// The generated SKILL.md is downloaded and read INSIDE the adopting repo, and its footer tells the
// reader to re-scan through that link. A self-hostable AGPL build must not send them to ascent.dev,
// where they have no account and no scan - the same rule lib/site states for SOURCE_REPO_URL.
describe("footer back-link follows the deployment, never a hardcoded product domain", () => {
  ...
  it("links the configured origin when one is set", () => {
    process.env.ASCENT_PUBLIC_URL = "https://maturity.acme.internal";
    const body = buildOnboardingSkill(makeReport({})).body;
    expect(body).toContain("[Ascent](https://maturity.acme.internal)");
  });

  it("degrades to UNLINKED attribution when no public origin is configured", () => {
    const body = buildOnboardingSkill(makeReport({})).body;
    expect(body).toContain("_Generated by Ascent,");
    expect(body).not.toContain("ascent.dev");
    expect(body).not.toContain("[Ascent](");
  });

  it("names the in-product control matrix after --json report-back when a public origin is set", () => {
    process.env.ASCENT_PUBLIC_URL = "https://maturity.acme.internal";
    const body = buildOnboardingSkill(makeReport({})).body;
    const matrixUrl = `https://maturity.acme.internal${orgTabHref("acme", "passports")}`;
    expect(body).toContain(`After \`--json\` report-back, the org control matrix is ${matrixUrl}.`);
    expect(body.match(/org control matrix is /g)).toHaveLength(1);
  });

  it("omits the control-matrix line when no public origin is configured", () => {
    const body = buildOnboardingSkill(makeReport({})).body;
    expect(body).not.toContain("org control matrix");
    expect(body).not.toContain("tab=passports");
    expect(body).not.toContain("conformance/matrix");
  });
});
