...
describe("proposeLaneKind — the craft rung", () => {
  it("proposes a CRAFT lane when the batch is all craft (i.e. no open gaps left)", async () => {
    const plan = await proposeLaneKind("C:/repo", async () => [craft("performance"), craft("robustness")]);
