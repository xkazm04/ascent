...
export function signBriefingShareToken(p: BriefingShareParams, ttlMs: number = DEFAULT_TTL_MS): { token: string; expiresAt: number } | null {
  ...
  const payload = Buffer.from(
    JSON.stringify({ org: p.org.toLowerCase(), range: p.range, from: p.from, to: p.to, winStart, winEnd, segment: p.segment, stack: p.stack, mintedBy: p.mintedBy, exp: expiresAt }),
  ).toString("base64url");
