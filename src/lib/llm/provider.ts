...
export interface LlmScoreInput {
  ...
  /** The deterministic Security (D9) check battery + its scores. D9's number is FIXED (deterministic);
   *  this is threaded so the LLM narrates it (summary + prioritized gaps) from the same graded checks
   *  rather than re-deriving it. Null when scanned without a token. */
...
const cap = (s: string): string => {
  ...
  if (clean.length <= MAX_FIELD_LEN) return clean;
  const sliced = clean.slice(0, MAX_FIELD_LEN);
