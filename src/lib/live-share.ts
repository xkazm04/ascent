...
//
// The dedicated-secret resolution is still shared with lib/briefing-share.ts via lib/signed-share.ts
// (resolveShareSecret); the token framing is domain-separated here and intentionally does NOT reuse the
// generic signShareToken/verifyShareToken codec (those sign the bare payload with no DOMAIN prefix).

import { createHmac, randomUUID, timingSafeEqual } from "node:crypto";
import { resolveShareSecret } from "./signed-share";
