...
export interface RepoContext {
   ...
   *
   * Present ⇒ the artifact carries a section built from the structure the org's own repos already
   * agree on, and the PR says so. Absent ⇒ the generic starter, and the PR says THAT. The
   * distinction is the whole point: "your own pattern, from 3 repositories" and "a generic starter"
   * are very different claims to put in front of an engineer, and a PR that blurred them would
   * spend the credibility the mining exists to earn.
   */
  house?: { lines: string[]; exemplars: string[] } | null;
}
...
`;
  ...
  const house = ctx.house && ctx.house.lines.length > 0 ? ctx.house : null;
  if (house) {
    body += `

## Your organization's shared pattern
${house.exemplars.length} of your repositories already structure this the same way (${house.exemplars.map(safeText).join(", ")}). Their shared sections:
