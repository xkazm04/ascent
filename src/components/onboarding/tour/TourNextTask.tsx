...
export function TourNextTask({ item, href, onShowMe }: { item: DrawerItem; href: string; onShowMe: () => void }) {
  return (
    <div className="rounded-xl border border-accent/40 bg-accent/[0.06] px-3.5 py-3">
      ...
      <h3 className="mt-1 type-body-sm font-semibold text-white">{item.title}</h3>
      <p className="mt-1 type-note leading-relaxed text-slate-300">{item.body}</p>
      {item.detail && <p className="mt-1.5 type-caption leading-relaxed text-slate-500">{item.detail}</p>}
      <div className="mt-3 flex items-center gap-2">
        <Link
          href={href}
          className="focus-ring rounded-lg bg-accent px-3 py-1.5 type-body-sm font-semibold text-on-accent transition hover:bg-accent-soft"
        >
          {item.cta}
        </Link>
        {item.tour.anchor && (
          <button
            type="button"
            onClick={onShowMe}
            className="focus-ring rounded-lg border border-slate-700 px-3 py-1.5 type-body-sm text-slate-300 transition hover:border-accent hover:text-white"
          >
            Show me
          </button>
        )}
