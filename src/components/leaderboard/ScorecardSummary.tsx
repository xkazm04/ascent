...
export function ScorecardSummary({ card }: { card: PublicOrgScorecard }) {
  if (card.verifiedCount === 0) {
    return (
      <div className="mt-8 rounded-2xl border border-amber-500/30 bg-amber-500/5 p-8">
        ...
        <p className="mt-2 max-w-2xl type-body leading-relaxed text-slate-400">
  ...
  return (
    <div className="mt-8 rounded-2xl border border-divider bg-surface/40 p-8">
      <div className="flex flex-wrap items-end gap-x-10 gap-y-6">
        <div>
          ...
          <div className="flex items-baseline">
            ...
            <span className="type-heading font-semibold text-slate-500">/100</span>
          </div>
          <span className="mt-1 flex items-center gap-2 type-mono-sm font-bold" style={{ color: levelHex }}>
        ...
        <dl className="flex gap-8">
          {[
            ...
            <div key={s.label}>
              <dt className="font-mono type-micro uppercase tracking-[0.2em] text-slate-500">{s.label}</dt>
              <dd className="mt-1 type-figure font-bold text-white">{s.value}</dd>
            </div>
          ))}
        </dl>
      </div>
        ...
        {DIMS.map((d) => {
          ...
          return (
            <div key={d} className="text-center" title={DIMENSION_BY_ID[d].name}>
              <span className="block font-mono type-micro uppercase tracking-wider text-slate-500">
                {DIMENSION_SHORT[d]}
              </span>
              <span
                className="mt-1 block font-mono type-lede font-bold tabular-nums"
      ...
      <p className="mt-6 border-t border-divider pt-4 type-note leading-relaxed text-slate-500">
