...
// The Dimensions explorer — the per-dimension breakdown, on the brand's own vertical. The nine
// dimensions stand on ONE elevation gauge (AltimeterGauge) whose strata are the five maturity bands,
// each with a hollow marker where the last scan left it; the climber list beneath carries the facts
// a reader needs to decide where to invest (level, weighted headroom "in reach", since-last delta,
// score) and a sort that re-orders gauge and list together. The detail column opens with a four-cell
// reading strip (level, next rung, in reach, model vs detectors) over the shared evidence body.
//
// Replaced the radar + score-bar pairing: a radar reads "shape" but never "height", and height
// against the level bands is the question this section exists to answer.
...
export function DimensionExplorer({
  ...
  // A scan where nothing could be scored ships an empty `dimensions` array (see ScanReport) — say so
  // rather than crashing on dims[0].
  if (!sel || !summary) {
    return (
