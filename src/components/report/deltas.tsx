...
export function DeltaPill({
  ...
  if (delta === 0) {
    return (
      <span
        className={`rounded-full border border-slate-600/40 bg-slate-500/10 px-2.5 py-1 type-body-sm text-slate-300 ${className}`}
      >
  ...
  return (
    <span
      className={`rounded-full border px-2.5 py-1 type-body-sm font-semibold ${
        up
...
export function DeltaTag({
  ...
  if (delta === 0) {
    ...
    return <span className={`type-body-sm font-semibold text-slate-500 ${className}`}>—</span>;
  }
  const up = delta > 0;
  return (
    <span
      className={`type-body-sm font-semibold tabular-nums ${up ? "text-emerald-400" : "text-red-400"} ${className}`}
    >
      {up ? "▲+" : "▼"}
      {delta}
    </span>
