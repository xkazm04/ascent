...
import { rewriteLiveScanAddressBar } from "@/components/report/liveScanPermalink";
...
export function ReportClient({ repo: repoProp }: { repo?: string } = {}) {
  ...
  // After persist, rename `/report?repo=` to the durable permalink in place. Withheld when the
  // snapshot was not saved (DB off, scoped, degraded) so a reload cannot unfurl ColdScanGate
  // under a URL that looks like a finished report.
  useEffect(() => {
    if (state.status !== "done") return;
    rewriteLiveScanAddressBar({
      pathname: window.location.pathname,
      search: window.location.search,
      fullName: `${state.report.repo.owner}/${state.report.repo.name}`,
      persisted,
      scoped: Boolean(ref || subPath),
    });
  }, [state, persisted, ref, subPath]);
