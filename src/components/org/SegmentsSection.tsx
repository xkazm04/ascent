// The Segments view, extracted from its former standalone route so it can render as the "Segments" tab
// of the consolidated Repositories page (the two Fleet views merged into one page). Server component:
// it owns its own reads (segment summaries + A/B comparison) so the host page stays a thin tab shell.
// The FleetTabs bar and the outer spacing are supplied by the host page, not here.
...
import { Card, Meter, SectionEmpty, SectionHeader, Tile, TILE_GRID, POSTURE_LABEL, deltaHex, fmtDelta } from "@/components/org/ui";
