...
// forcing one would ripple far outside this agent's scope (src/components/org/govern/audit/**).
//
...
// Resolved from the repo root rather than by counting `..` segments off this file's own depth: the
// org-tabs refactor moved this folder one level deeper (org/audit → org/govern/audit) and the old
// relative walk silently pointed at `src/src` and failed. vitest runs with cwd = repo root.
