...
// §6). Click a header to sort; click again to flip. Default: production score, descending. Reuses the
...
export function PassportTable({ rows }: { rows: PassportRow[] }) {
  ...
  return (
    <OrgTable
      caption="Fleet passport portfolio: automation and production readiness per repo"
      minWidth={760}
      head={
        <tr>
      ...
      {sorted.map((r) => (
        <tr key={r.fullName} className="text-slate-300">
          <td className="px-3 py-2">
            <Link href={`/report?repo=${encodeURIComponent(r.fullName)}`} className="font-mono text-sm text-white hover:text-accent">{r.name}</Link>
          </td>
