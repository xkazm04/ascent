...
import { Card, postureLabel, deltaHex, fmtDelta } from "@/components/org/shared/ui";
...
function SideLinks({ org, s }: { org: string; s: SegmentSummary }) {
  ...
  const repoHref = stackQ ? `${base}${base.includes("?") ? "&" : "?"}${stackQ.slice(1)}` : base;
  return (
    <span className="text-slate-500">
      <span className="text-slate-300">{s.name}</span>:{" "}
      <Link href={repoHref} className="text-accent transition hover:text-white">repos</Link>
      {" · "}
