// The credit matrix's capability rows are DERIVED from the plan model (src/lib/plans.ts), not typed
// alongside it. This pins the join: every gated capability appears exactly once in the matrix, and
// every cell says what the entitlement gate would say for that tier.
//
// Before the derivation, these rows were hand-written with hand-written `from(tier)` cells, so the
// matrix could promise a tier a capability the gate refused — and two gated capabilities (Shared org
// memory, PDF export) were missing from the table altogether, sold nowhere and enforced anyway.

import { describe, it, expect } from "vitest";
import { PLAN_CAPABILITIES, PLAN_CAPABILITY_ORDER, PLAN_FEATURES, PLAN_ORDER, planAllows } from "@/lib/plans";
import { CREDIT_RULE, MATRIX_GROUPS, MATRIX_PLANS, PLANNED_ROW_LABELS } from "./creditMatrixData";

const capabilitiesGroup = MATRIX_GROUPS.find((g) => g.key === "capabilities")!;
const scanningGroup = MATRIX_GROUPS.find((g) => g.key === "scanning")!;
const rowFor = (label: string) => capabilitiesGroup.rows.find((r) => r.label === label);
const anyRow = (label: string) => MATRIX_GROUPS.flatMap((g) => g.rows).find((r) => r.label === label);

describe("credit matrix — the capability rows are the gate, rendered", () => {
  it("lists every gated capability, once, under its model label", () => {
    for (const cap of PLAN_CAPABILITY_ORDER) {
      const label = PLAN_CAPABILITIES[cap].label;
      const matches = capabilitiesGroup.rows.filter((r) => r.label === label);
      expect(matches, label).toHaveLength(1);
    }
  });

  it("every cell agrees with planAllows() for that tier — the page cannot oversell the gate", () => {
    // The suite runs with ASCENT_SELF_HOSTED=0 pinned (vitest.config.js), so this is the CLOUD matrix,
    // which is the one /pricing is describing. A self-hosted build has no pricing page to contradict.
    for (const cap of PLAN_CAPABILITY_ORDER) {
      const row = rowFor(PLAN_CAPABILITIES[cap].label)!;
      for (const plan of PLAN_ORDER) {
        expect(row.cells[plan], `${cap} @ ${plan}`).toBe(planAllows(cap, plan));
      }
    }
  });

  it("marks capability rows as included-in-plan, never as credit-metered", () => {
    // The whole point of the matrix: credits buy exactly one thing (a scan past the allowance).
    for (const cap of PLAN_CAPABILITY_ORDER) expect(rowFor(PLAN_CAPABILITIES[cap].label)!.tag).toBe("plan");
  });

  it("still carries the ungated rows the model deliberately does not gate", () => {
    // Segments/Playbooks are advertised but enforced nowhere (docs/features/billing/billing.md).
    // They stay hand-written rows precisely so the derived ones mean "enforced".
    expect(rowFor("Segments + comparisons")).toBeDefined();
    expect(rowFor("Playbooks + planning")).toBeDefined();
  });

  it("keeps one column per tier, in ladder order", () => {
    expect(MATRIX_PLANS.map((p) => p.id)).toEqual(PLAN_ORDER);
  });
});

// The scanning group used to open with "Every scan, public or private, draws on one monthly
// allowance", three lines under a file header that says the opposite and directly above a table that
// shows the opposite. `plans.ts` (PlanFeature.includedCredits doc), `db/credits.ts:3` and
// `/pricing`'s own page header all agree: an anonymous PUBLIC scan is never metered.
describe("credit matrix — public scans are never metered, and the copy says so", () => {
  it("gives public and private scans their own rows, with only the private one drawing credits", () => {
    const pub = scanningGroup.rows.find((r) => r.label === "Public repository scan")!;
    const priv = scanningGroup.rows.find((r) => r.label === "Private repository scan")!;
    expect(pub.tag).toBe("free");
    expect(priv.tag).toBe("credit");
    // Free on EVERY tier, with no per-tier number to overrun.
    for (const plan of PLAN_ORDER) expect(pub.cells[plan], `public @ ${plan}`).toBe("Unlimited");
  });

  it("never lumps public in with private, and names public scans in both summary sentences", () => {
    // "Every scan, public or private, …" is the exact phrasing that was wrong; no summary may
    // put the two kinds on the same side of the metering rule again.
    for (const text of [scanningGroup.intro, CREDIT_RULE]) {
      expect(text, text).not.toMatch(/public or private/i);
      expect(text, text).toMatch(/public scans/i);
    }
  });

  it("reads the private allowance cells off the plan model, so they cannot drift from the gate", () => {
    const priv = scanningGroup.rows.find((r) => r.label === "Private repository scan")!;
    for (const plan of PLAN_ORDER) {
      const n = PLAN_FEATURES[plan].includedCredits;
      expect(priv.cells[plan], `allowance @ ${plan}`).toBe(n == null ? "Your volume" : `${n} / mo`);
    }
  });
});

// `docs/features/billing/billing.md:424-438` audited every `planAllows*` predicate and every plan
// comparison in the tree and listed the tier claims with NO CODE GATE behind them. The matrix ticked
// them anyway — a page asserting a restriction the product does not apply. They now carry `planned`.
// This test is the join: the audit's list is the flag's list, in both directions.
describe("credit matrix — an unenforced tier boundary is marked, not asserted", () => {
  const UNENFORCED = [
    "Org fleet dashboard",
    "Scheduled autoscans",
    "Regression + credit alerts",
    "Segments + comparisons",
    "Playbooks + planning",
    "Buy extra scan credits",
    "Members / seats",
  ];

  it("marks every claim billing.md found ungated", () => {
    for (const label of UNENFORCED) expect(anyRow(label)?.planned, label).toBe(true);
  });

  it("marks NOTHING else — a flag on an enforced row would understate what the plan really buys", () => {
    expect([...PLANNED_ROW_LABELS].sort()).toEqual([...UNENFORCED].sort());
  });

  it("never flags a row derived from PLAN_CAPABILITIES — those cells ARE the gate", () => {
    for (const cap of PLAN_CAPABILITY_ORDER) {
      expect(rowFor(PLAN_CAPABILITIES[cap].label)!.planned, cap).toBeUndefined();
    }
  });

  it("leaves retention unflagged — the one per-tier number the code really reads", () => {
    expect(rowFor("Scan history")!.planned).toBeUndefined();
    for (const plan of PLAN_ORDER) expect(PLAN_FEATURES[plan].retentionDays !== undefined).toBe(true);
  });
});
