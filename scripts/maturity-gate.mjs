...
//
// WHICH OF THOSE ACTUALLY BITE HERE. /api/gate is unauthenticated, so its scan runs WITHOUT a GitHub
// token, and branch governance + pull-request statistics both need one. `--require-protection`,
// `--min-ai-governed` and `--no-ungoverned-ai` therefore have nothing to test on this endpoint and
// are reported back in the response's `skipped[]` — which this script PRINTS, on a pass as loudly as
// on a failure, and exposes as the `skipped` step output. Advertising a flag that silently does
// nothing is how a bar becomes decorative; those bars land on the GitHub App's check run, which
// scans with the installation's own token.
//
...
/**
 * The conditions the verdict could NOT test, as printable lines (empty when everything was measured).
 *
 * Pure and exported so it can be unit-tested: this is the half of the verdict that is easiest to lose
 * — nothing fails when it goes missing, the build just stays green on a bar nobody checked.
 */
export function formatSkipped(skipped) {
  const list = Array.isArray(skipped) ? skipped.filter((s) => s && typeof s.code === "string") : [];
  if (!list.length) return [];
  const lines = ["  NOT MEASURED on this run — these conditions were configured but never tested:"];
  for (const s of list) lines.push(`  - ${s.code}: ${s.why ?? "no reason given"}`);
  lines.push("  A skipped condition is not a pass. See the gate docs for which surface can enforce it.");
  return lines;
}

/** The deduped skip codes, as the single-line scalar the `skipped` step output must be. */
export function skippedCodes(skipped) {
  const list = Array.isArray(skipped) ? skipped : [];
  return [...new Set(list.map((s) => s?.code).filter((c) => typeof c === "string"))].join(",");
}
...
const outcome = (status, body = {}) =>
  setOutputs({
    ...
    // Which configured conditions this verdict could not test. A workflow that requires a bar to have
    // been ENFORCED (not merely requested) can now branch on it — `pass: true` alone cannot say.
...
async function main() {
  ...
  try {
    ...
    // A throttle means the repo was NEVER SCANNED, so it belongs with 404/503 in the "gate could not run"
    ...
    if (body.pass) {
      ...
      // Printed on the PASS path too, and that is the whole point: a green build is exactly where an
      // untested bar is invisible, and the operator who asked for --require-protection deserves to
      // learn here that nothing checked it.
      for (const line of formatSkipped(body.skipped)) console.log(line);
      outcome("pass", body);
      process.exit(0);
    }
    console.error(`✖ Maturity gate FAILED — ${head}`);
    for (const f of body.failures ?? []) console.error(`  - ${f.message}`);
    for (const line of formatSkipped(body.skipped)) console.error(line);
    outcome("fail", body);
    process.exit(1);
  } catch (err) {
...
// Run only when INVOKED as the CLI, so the pure formatters above can be imported by a unit test
// without the import itself firing a gate call and calling process.exit(). `action.yml` and
// `npm run gate` both invoke this file directly, so the guard is transparent to every real caller.
