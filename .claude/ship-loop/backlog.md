...
| 11 | ☑ | 8-Ops | M | DONE (2026-08-28, ship/ascent-stabilize): `ci.yml` gains an `e2e` job parallel to `verify` — cached chromium (`actions/cache` on the resolved @playwright/test version) + `playwright install --with-deps chromium`, then two suites: the public funnel (`playwright.config.ts`, `--grep-invert @livescan`, `e2e/scan.spec.ts e2e/connect`) and the local-mode loop (`npm run test:e2e:loop`). Traces uploaded on failure. EXCLUDED with reasons in the job's header comment: `e2e/org-suite/**` (needs a seeded Postgres `vercel` org + a live model), and the two new `@livescan` specs (real GitHub ingest — an external dependency a blocking gate should not carry; both pass locally). Wiring it surfaced a stale `@smoke` assertion red since 7deeaa84 (2026-08-14) — the pricing h1 — fixed in the same commit. |
