...
> v1.3/v1.4). Source run: `uat/runs/2026-08-10-ascent-first/`, with 3 Characters (Sam, Dana, Tomáš),
...
carry `resolution: null`, so the drain had to read severity/`impact` and the reports to place them.
...
**−6 pts**"*, because `valueRealizedLine` pushes `pointsMoved` sign-blind (`briefing.ts:58`). Beside it,
...
existing but unused field) and the one with the clearest payoff, since it converts a conditional pass into
...
and also the roadmap, and the discrepancies block.
