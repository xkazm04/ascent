...
- `src/lib/scoring/engine.ts:67-74, 151-161` adds the **D9 visibility escape hatch**: a high-confidence,
...
2. **It only exercises the deterministic layer.** `analyzeSignals` + `overallScoreFor` are called
   ...
   free text** (`engine.ts:68`), which cannot be validated by a signal-layer unit test.
...
rather than repo snapshots, so this requires a live re-fetch, roughly 110 scans.
...
| Genuine re-score, healthy full-coverage scan | **0 overall, ±1 per dimension**, the one empirical measurement from `noise.ts:5-8` (two independent claude-cli re-scans, UAT pricing-20, 2026-06-20). **n=1.** |
...
1. **The D9 escape hatch is a regex coin-flip.** At `engine.ts:67-74`, whether the model happens to write
...
- **Mock/deterministic-only scans count as peers.** The seeder defaults to `mock=true`
  ...
  Briefing already renders an `engineMixCaveat` (`briefing.ts:32`), but the benchmark ignores it.
...
every percentile already shipping in the Briefing PDF and on `/portfolio`, which is a live correctness
...
- A named "Fast & Ungoverned" panel listing the repos, each deep-linking to its report and to
  `/org/[slug]/governance`, in `src/components/org/executive/UngovernedPanel.tsx`.
...
- `buildGovernanceOverview(orgSlug, segmentId, techGroupId)` (`:101`) evaluates every scanned repo
...
- `greenPath: GreenPathItem[]` / `closestToGreen` (`:34, :198`): failing repos ranked by
  ...
  (level / posture).
- `governanceMarkdown` (`:218`) and `ciActionYaml` (`:212`) are **exportable already**.
...
better home, since it is the same data with a different question). Ranked table: tier chip, repo, reasons,
...
branch-protection is unreadable, so the policy must then **not** fire `requireProtectedBranch` (the
