...
- ✅ **P0-3 · D9 make GitHub-native checks `n/a`, not `0`, without GitHub CI** (SAST/SBOM/signed-releases/dependency-updates); add off-GitHub detectors (`govulncheck`, `cargo-audit`/`cargo-deny`); fix the token-permissions regex `^permissions:` → `^\s*permissions:` (currently misses job-level least-privilege blocks). — `src/lib/security/checks.ts`
- ✅ **P0-4 · Teach D6/D7 + branch-protection to read merge-queue/Gerrit review** from commit trailers (`r=`, `Reviewed-on:`, `Change-Id`, `TryBot-Result`) so "0% reviewed / nothing stops a merge" stops firing on stricter-than-GitHub gates. — `src/lib/analyze/pulls.ts` (~91,132)
...
- ✅ **P1-5 · Persist `warnings`.** Add `warningsJson` column + write on persist + merge with the recomputed stack-fit caveat on read. Stops degraded/low-coverage/no-token scans reading as confident full scans on reload. — `prisma/schema.prisma` (Scan), `scans-persist.ts`, `scans-read.ts`
