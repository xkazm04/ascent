...
- **`rescanCadence` is not persisted on a manual run.** The setup dialog's Rescan dial reaches
  manual runs, drives, and the standing runner; a drive stores its dials in `LoopDrive.dialsJson`.
  Nothing on the manual `LoopRun` row records which cadence a finished run used, so the ledger
  cannot yet answer "was this run's pair a per-cycle or a per-run reading" except by the lane logs.
