...
| Chosen | Care | `?tab=care` | `src/components/org/library/care/` | UC3 individual care. ONE tab id, TWO modes by `Organization.kind`: personal mode is the developer's own home (profile, moves board, session shape, my repos' gaps, journal, privacy ledger); org mode is the anonymized aggregate under `CHAMPION_MIN_POP`, never a per-person row. Currently a **prototype round** with three variants behind a switcher and an honest empty live path — see [care.md](care.md). |
