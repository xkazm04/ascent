...
| `getOrgGovernance` / `getOrgActivity` / `getOrgPrSignals(slug)` | Delivery-tab aggregates (point-in-time: each repo's latest scan). **Pooled fleet rates (2026-09-23):** review coverage, AI PRs reviewed, small PRs, AI involvement and revert rate are the fleet's summed counts over its summed population from each scan's rate book (`PrStats.rates`), floored by `RATE_BASIS.minSample` on the pooled population (`src/lib/db/fleet-rate-pool.ts`), so 1 of 10 plus 10 of 10 human-merged PRs reads 55%, not the 18% an analyzed-weighted mean gave. A rate stays analyzed-weighted (no partial pool) while any repo's latest scan predates the book, and merge / AI trailer / AI pre-review rates, which persist no counts, are always weighted. `rateBasis[id]` carries `method`, `count` and `legacyRepos`, and the band's basis line reads "basis: 11 of 20 · 2 repos" or says the rate is weighted because N scans predate per-rate counts. The Adoption tab's delivery strip reads the same figures. `getOrgRework` still folds with the analyzed weighting (no consumer yet). **Review integrity (2026-09-23):** `PrRepoRow.integrity` passes each repo's `selfApproved` / `fastApproval` counts through from the rate book (null per entry for a scan that predates it); no fleet `avg*Rate` reads them. On the Delivery tab a `ReviewIntegrityStrip` sits directly under the PR band (`#review-integrity`): the fleet's self-approved share (over human-authored merged PRs) and approved-within-5-minutes share (over approved PRs, self-approvals excluded), each pooled through `poolFleetRate` and floored at `REVIEW_INTEGRITY_MIN_SAMPLE` (5) on the pooled population (`src/features/bought/delivery/reviewIntegrityModel.ts`), with its basis ("12 of 25 approved PRs") visible and the `RATE_BASIS` caveat on a WhyChip beside each label. A repo without the counts is left out of that pool and named ("1 repo predates these counts"); a fleet where no scan carried them renders "not measured in these scans", never 0%. Repos with at least 5 approved PRs whose fast share is 50% or more are listed as questions linking to the per-repo rows, and "Fix first" adds one improve item, "Ask what approval means in N repos", even when review coverage clears 80%, so a fleet of instant approvals no longer gets the all-clear. Every sentence is about a repository's approvals; no reviewer is named. |
| `getOrgDeliveryTrend(slug, window, segmentId?, techGroupId?)` (`src/lib/db/org-delivery-trend.ts`) | **Delivery over time (2026-07-28).** The Delivery tab's only *windowed* read: it walks every `Scan` in the period and folds the already-persisted `prStats`/`governance` blobs into one point per canonical-zone calendar day. Rates fold exactly like `getOrgPrSignals` (pooled from the rate book on a day when every scan carries it, `basis[metric].method`/`count`/`population`; analyzed-PR-weighted otherwise); a nullable rate stays null ("no sample" ≠ a measured 0); unreadable governance contributes nothing. Each point carries its own `scans`/`repos`/`prs` sample size, because a point describes **the repos scanned that day, not the fleet**, the same semantics as the maturity trend, disclosed rather than reconstructed. Lower bound is retention-clamped like the maturity trend. `fits` gives a per-week slope for `reviewedRate` / `aiGovernedRate` / `hoursToFirstReview` (the last in hours/week, the W1a review-latency delta), gated by the **shared** `forecastInsufficiency` floor and deliberately narrowed to the slope fields (a review-coverage percentage has no maturity level, so `projectedLevel`/`eta` are never exposed). Pure `buildDeliveryTrend` / `buildDeliveryRateFit` are unit-tested. |
